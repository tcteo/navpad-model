...
def mx_keyswitch_hole(x=None,y=None,z=None,xangle=None,yangle=None,offset=None, show_keycap_planes=False):
  with s.translate([x,y,z]):
    with s.rotate([yangle, xangle, 0]): 
      with s.translate([0,0,-offset]):
        with s.difference():
          with s.translate([-switch_size/2-switch_border_x, -switch_size/2-switch_border_y, -zheight+ zfe]):
            s.cube([switch_size+2*switch_border_x, switch_size+2*switch_border_y, zheight-2*zfe]);
...
def main():
  ...
  with m:
    # switch_hole_xyplane_centered()
    # mx_keyswitch_hole(x=0, y=0, z=0, xangle=)
    
    for col in range(0,5):
      colx = col * x_spacing
      # bottom row
      mx_keyswitch_hole(x=colx,y=0,z=0,xangle=0,yangle=0,offset=13.6)
      # middle row
      mx_keyswitch_hole(x=colx,y=17.5,z=1.5,xangle=0,yangle=10,offset=13.6)
      # top row
      mx_keyswitch_hole(x=colx,y=34,z=6,xangle=0,yangle=22,offset=13.6)
